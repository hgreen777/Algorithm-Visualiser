...
import time
import pygame
from Algorithms import linearSearch, binarySearch, jumpSearch, exponentialSearch, fibonacciSearch, bogoSearch, bubbleSort, insertionSort, mergeSort, heapSort, countingSort, bogoSort, quickSort, selectionSort, radixSort
import random
import math
import numpy as np
...
def updateVisual(arr, selected, metrics):    
    ...
    # Play sound
    if not disable_sound and len(selected) != 0:
        try:
            frequency = value_to_frequency(arr[selected[0]])
        except:
            frequency = value_to_frequency(arr[0])
